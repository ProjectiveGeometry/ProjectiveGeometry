Require Import ords ssreflect ssrfun ssrbool pg3x_spec pg32_inductive.
Require Import wlog.
...
Module PS : ProjectiveSpace.
  ...
  Ltac prove_a3_1 := let l := fresh "l" in
                     ...
                     case l; intros x;
                     exists_p3 (fst (fst x)) (snd (fst x)) (snd x).
...
(* a3_3 : given 3 lines, there exists a line which intersects these 3 lines *)
  ...
  Lemma a3_3_simple :
   ...
   Proof.
     ...
     par:abstract (time (case v2; intros hp1p2; first [exact (degen_bool _ hp1p2) | 
                                             
               (case v3;
               intros hp1p3 hdist x;
               solve [ (exact (degen_bool _ hp1p3)) | (exact (degen_bool _ hdist)) |
                       exists_lppp (fst x) (fst (fst (snd x))) (snd (fst (snd x))) (snd (snd x))
              ])])).
     Time Qed.
   (*368s*)
   Check a3_3_simple.
