Require Export ProjectiveGeometry.Dev.rank_space_properties.

(*****************************************************************************)
(** Proof of projective space geometry to matroid' **)


Section s_rkEquivToPs.

Context `{RPSOH : RankProjectiveSpace}.
...
(*** Definition Intersection_In ***)
Definition Intersect_In (l1 l2 : Line) (P : Point) := Incid P l1 /\ Incid P l2.

Lemma a3_3 (* dim = 3 *) : forall l1 l2 l3 : Line, ~ line_eq l1 l2 /\ ~ line_eq l1 l3 /\ ~ line_eq l2 l3 -> 
...
unfold line_eq in *.
simpl in *.
assert(HH := rk_quadruple_3_or_higher A B A0 B0 H3 H4 H0).
assert(HH0 : rk (quadruple A B A0 B0) >= 3).
omega.
assert(HH1 := rk_quadruple_3_or_higher A B A1 B1 H3 H5 H1).
assert(HH2 : rk (quadruple A B A1 B1) >= 3).
omega.
assert(HH3 := rk_quadruple_3_or_higher A0 B0 A1 B1 H4 H5 H2).
assert(HH4 : rk (quadruple A0 B0 A1 B1) >= 3).
omega.
assert (HH5 : rk (couple A B) = 2).
apply rk_couple1;assumption.
assert (HH6 : rk (couple A0 B0) = 2).
apply rk_couple1;assumption.
assert (HH7 : rk (couple A1 B1) = 2).
apply rk_couple1;assumption.
assert(HH8 : rk (couple A B) = 2 /\ rk (couple A0 B0) = 2 /\ rk (couple A1 B1) = 2 /\
rk (quadruple A B A0 B0) >= 3 /\ rk (quadruple A B A1 B1) >= 3 /\ rk (quadruple A0 B0 A1 B1) >= 3).
intuition.

assert(HH9 := rk_upper_dim A B A0 B0 A1 B1 HH8).
destruct HH9.
destruct H6.
destruct H6.
destruct H6.
...
case_eq(eq_dec x x0);
case_eq(eq_dec x x1);
case_eq(eq_dec x0 x1).

(* cas 1 *)

intros.
assert(HH10 := a1_exists x x0).
destruct HH10.
destruct H13.
exists x2.
exists x.
...
split.
assumption.
clear H0 H1 H2 HH HH0 HH1 HH2 HH3 HH4 HH5 HH6 HH7 HH8 H10 H11 H12.

assert(HH9 := rk_couple1 x x0 n1).
assert(HH10 := rk_couple1 x x1 n0).
assert(HH11 := rk_couple1 x0 x1 n).
assert(HH12 := rk_line_unification x x0 x1 HH9 HH10 HH11 H9).
induction x2.
simpl in *.

assert (HH13 : rk (union (triple A2 B2 x) (triple A2 B2 x0)) + rk (couple A2 B2) <=
           rk (triple A2 B2 x) + rk (triple A2 B2 x0)).
apply (matroid3_useful (triple A2 B2 x) (triple A2 B2 x0) (couple A2 B2)).
clear_all;fsetdecide.

assert(HH14 := rk_couple1 A2 B2 H0).
assert(HH15 : rk (triple A2 B2 x ++ triple A2 B2 x0) <= 2).
omega.
assert(HH16 : (triple A2 B2 x ++ triple A2 B2 x0 [=] quadruple A2 B2 x x0)%set).
clear_all;fsetdecide.
rewrite HH16 in HH15;clear HH16.
assert(HH17 : (couple A2 B2 [<=] quadruple A2 B2 x x0)%set).
clear_all;fsetdecide.
assert(HH18 := matroid2 (couple A2 B2) (quadruple A2 B2 x x0) HH17).
assert(HH19 : rk(quadruple A2 B2 x x0) = 2).
omega.

clear HH17.

assert (HH20 : rk (union (quadruple A2 B2 x x0) (triple x x0 x1)) + rk (couple x x0) <=
           rk (quadruple A2 B2 x x0) + rk (triple x x0 x1)).
apply (matroid3_useful (quadruple A2 B2 x x0) (triple x x0 x1) (couple x x0)).
clear_all;fsetdecide.
assert(HH21 : rk(quadruple A2 B2 x x0 ++ triple x x0 x1) <= 2).
omega.
assert(HH22 : (quadruple A2 B2 x x0 ++ triple x x0 x1 [=] quadruple A2 B2 x x0 ++ singleton x1)%set).
clear_all;fsetdecide.
rewrite HH22 in HH21;clear HH22.
assert(HH23 : (couple A2 B2 [<=] union (quadruple A2 B2 x x0) ({x1}))%set).
clear_all;fsetdecide.
assert(HH24 := matroid2 (couple A2 B2) (union (quadruple A2 B2 x x0) ({x1})) HH23).
assert(HH25 : rk(union (quadruple A2 B2 x x0) ({x1})) = 2).
omega.
clear HH23 HH24.
assert(HH26 : (couple A2 B2 [<=] (triple A2 B2 x1))%set).
clear_all;fsetdecide.
assert(HH27 := matroid2 (couple A2 B2) (triple A2 B2 x1) HH26).
clear HH26.
assert(HH28 : (triple A2 B2 x1 [<=] (union (quadruple A2 B2 x x0) ({x1})))%set).
clear_all;fsetdecide.
assert(HH29 := matroid2 (triple A2 B2 x1) (union (quadruple A2 B2 x x0) ({x1})) HH28).
omega.
Qed.
