...
Lemma rk_upper_dim_aux : forall a b c d,
contains_three_non_collinear_points (a :: b :: c :: d :: nil) = true ->
collinear a b c = false \/ collinear a b d = false \/ collinear a c d = false \/ collinear b c d = false.
Proof.
intros.
rewrite contains_1 in H.
destruct H.
...
inversion H;subst.
inversion H0;subst.
assert(HH := col_1 a x1).
rewrite H2 in HH.
inversion HH.
inversion H4;subst.
inversion H1;subst.
assert(HH := col_1 a b).
rewrite col_exchange2 in HH.
rewrite H2 in HH.
inversion HH.
inversion H5;subst.
assert(HH := col_2 a b).
...
Lemma rk_upper_dim_aux2 : forall a b c, forall x,
Incid a x -> Incid b x -> Incid c x -> collinear a b c = false -> False.
Proof.
intros.
generalize H2.
unfold collinear.
...
intros.
destruct a0.
assert(HH := uniqueness a b x x0 H H0 H6 H7).
destruct HH.
intuition.
rewrite H8 in *.
contradiction.
...
Lemma rk_upper_dim_aux3 : forall a b c,
contains_three_non_collinear_points (a :: b :: c :: nil) = true ->
collinear a b c = false.
Proof.
intros.
rewrite contains_1 in H.
