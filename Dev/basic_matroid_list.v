...
- inversion H.
- simplgen H;case_eq(eq_dec a0 a);my_inA.
Qed.
...
Proof.
my_inA;induction l;my_inA.
- rewrite H1 in *;simplgen H;case_eq(eq_dec a a);my_inA.
- simplgen H;case_eq(eq_dec a0 a);my_inA.
Qed.
...
Proof.
my_inA;induction l;my_inA.
- simpl;case_eq(Inb a0 m);my_inA;assert(HH := Inb_aux2 a0 m H0);subst;my_inA.
- simpl;case_eq(Inb a0 m);my_inA.
Qed.
...
Ltac my_inA_inter :=
  ...
  |[H : _ = _ |- _] => rewrite <-H
  |[H : In _ nil |- _] => inversion H
  |[H : In _ (list_inter _ _) |- _] => apply list_inter_split in H
...
Ltac my_inA_union :=
  ...
  |[H : _ |- _] => progress intuition
  |[H : _ |- _] => split;intuition
  |[H : _ |- In _ (_ ++ _)] => apply in_app_iff
  |[H : In _ (_ ++ _) |- _] => apply in_app_iff in H
  |[H : In _ (?P ::  _ ) |- _] => inversion H;clear H
  |[H : _ = _ |- _] => rewrite <-H
  |[H : In _ nil |- _] => inversion H
  |[H : _ |- _] => progress apply in_cons
end.
